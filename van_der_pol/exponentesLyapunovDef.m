...
function [lambda] = exponentesLyapunovDef (mu_, A_, omega_, ci, N)
    ...
    [t, x] = ode45(@VanDerPolForzado, 0:dt:(N*dt), [x(end, 1); x(end,2); x(end,3)], opciones);
   ...
   % Bucle de cálculo
    for k = 1:N
        ...
        J = (eye(3)+[0 1 0 ; -1-2*x1*x2*mu mu*(1-x1^2) A*cos(x3) ; 0 0 0]*dt);
        w = J*w;        
