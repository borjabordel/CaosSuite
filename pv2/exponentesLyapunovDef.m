...
function [lambda] = exponentesLyapunovDef (alfa_, beta_, ci, N)
    ...
    % Integración
    [t, x] = ode45(@SistemaPV2,0:1:10, ci, opciones);
    [t, x] = ode45(@SistemaPV2, 0:dt:(N*dt), [x(end, 1); x(end,2); x(end,3)], opciones);
   ...
   % Bucle de cálculo
    for k = 1:N
        ...
        J = (eye(3)+[0 0 -alfa; beta 0 2*x3 ; 1 5/2  -2]*dt);
        w = J*w;        
